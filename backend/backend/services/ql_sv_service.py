...
class QLSinhVienService:
    ...
    async def get(self, ma_sv: str, ma_khoa: str = None):
        try:
            if ma_khoa:
                ...
                sinh_vien = await self.sv_repo.get(ma_sv=ma_sv)
            if not sinh_vien:
    ...
    async def get_all(self, ma_khoa: str = None) -> List[schemas.SinhVienResponse]:
        try:
            if ma_khoa:
                danh_sach_sv = await self.sv_repo.khoa_get_all(ma_khoa)
            else:
                danh_sach_sv = await self.sv_repo.get_all()
            if not danh_sach_sv:
