...
@router.get("/", response_model=List[detai_sv_schemas.DangKyNCKHResponse])
...
@router.get("/{ma_dk}", response_model=detai_sv_schemas.DangKyNCKHResponse)
...
@router.post("/", response_model=detai_sv_schemas.DangKyNCKHResponse)
...
@router.put("/{ma_dk}", response_model=List[detai_sv_schemas.NguyenVongDKResponse])
...
@router.delete("/{ma_dang_ky}")
async def delete_dk(ma_dang_ky: int, 
                    db: AsyncSession = Depends(get_db), 
                    current_user: dict = Depends(get_current_user)):
    service = DangKyNCKHService(db, current_user)
    return await service.delete(ma_dang_ky)