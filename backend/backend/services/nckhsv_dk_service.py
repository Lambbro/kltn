...
from models.base_models import DangKyNCKH, GiangVien, SinhVien, NguyenVongDK, hnc_dknckh_table, HuongNghienCuu
...
class DangKyService:
    ...
    async def get_all(self, ma_khoa: Optional[str] = None) -> List[DangKyNCKHSVResponse]:
        try:
            if ma_khoa:
                query = (
                    select(DangKyNCKH)
    ...
    #Lấy sinh viên theo giảng viên qua đăng ký nckh
    async def get_dssv_by_dk(self, ma_gv: str):
        try:
            query = (
                select(SinhVien.ma_sv, SinhVien.ten_sv)
                .join(DangKyNCKH, DangKyNCKH.ma_sv == SinhVien.ma_sv)
                .join(NguyenVongDK, NguyenVongDK.ma_dk == DangKyNCKH.ma_dk)
                .where(
                    DangKyNCKH.trang_thai == 1,
            ...
            return [{"ma_sv": ma_sv, "ten_sv": ten_sv} for ma_sv, ten_sv in records]
