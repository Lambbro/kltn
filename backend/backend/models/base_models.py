...
thanhvien_nhomSV_table = Table(
    ...
    Column("ma_nhom", Integer, ForeignKey("NhomNCKH.ma_nhom", ondelete="CASCADE"), primary_key=True),
    Column("ma_sv", String(20), ForeignKey("SinhVien.ma_sv", ondelete="CASCADE"), primary_key=True)
)
# Bảng Khen thưởng đề tài: Đề tài NCKH Sinh Viên - Khen Thưởng
khenthuong_detaiNCKHSV_table = Table(
    "KhenThuongDeTai", Base.metadata,
    Column("ma_de_tai", Integer, ForeignKey("DeTaiNCKHSV.ma_de_tai", ondelete="CASCADE"), primary_key=True),
    Column("ma_khen_thuong", Integer, ForeignKey("KhenThuong.ma_khen_thuong", ondelete="CASCADE"), primary_key=True)
)
# Bảng Người nhận thông báo: Thông báo - tài khoản
taikhoan_thongbao_table = Table(
    "NguoiNhanTB", Base.metadata,
    Column("email", ForeignKey("TaiKhoan.email", ondelete="CASCADE"), primary_key=True),
    Column("ma_tb", ForeignKey("ThongBao.ma_tb", ondelete="CASCADE"), primary_key=True)
)
#Bảng Hướng Nghiên Cứu của giảng viên: Giảng Viên - Hướng Nghiên Cứu
giangvien_hnc_table = Table(
    "GVHNC", Base.metadata,
    Column("ma_gv", String(20), ForeignKey("GiangVien.ma_gv", ondelete="CASCADE"), primary_key=True),
    Column("ma_hnc", Integer, ForeignKey("HuongNghienCuu.ma_hnc", ondelete="CASCADE"), primary_key=True)
)
#Bảng HNC đăng ký nckh: Hướng Nghiên Cứu - Đăng Ký NCKH
hnc_dknckh_table = Table(
    "HNCDangKyNCKH", Base.metadata,
    Column("ma_hnc", Integer, ForeignKey("HuongNghienCuu.ma_hnc", ondelete="CASCADE"), primary_key=True),
    Column("ma_dk", Integer, ForeignKey("DangKyNCKH.ma_dk", ondelete="CASCADE"), primary_key=True)
)
...
hnc_detaiNCKHGV_table = Table(
    ...
    Column("ma_hnc", Integer, ForeignKey("HuongNghienCuu.ma_hnc", ondelete="CASCADE"), primary_key=True),
    Column("ma_de_tai", Integer, ForeignKey("DeTaiNCKHGV.ma_de_tai", ondelete="CASCADE"), primary_key=True)
)
# MODEL CHÍNH
#model khoa
class Khoa(Base):
...
class TaiKhoan(Base):
    ...
    quyen_han = Column(Integer, nullable=False, default=4)

    sinh_vien = relationship("SinhVien", back_populates="tai_khoan", cascade="all, delete-orphan")
    giang_vien = relationship("GiangVien", back_populates="tai_khoan", cascade="all, delete-orphan")
...
class SinhVien(Base):
    ...
    dk_nckh = relationship("DangKyNCKH", back_populates="sinh_vien", cascade="all, delete-orphan")
...
class GiangVien(Base):
    ...
    tv_de_tai_gv = relationship("ThanhVienNCKHGV", back_populates="giang_vien")
...
class HoiDongKhoa(Base):
    ...
    tvhd_khoa = relationship("TVHDKhoa", back_populates="hd_khoa", cascade="all, delete-orphan")
...
class TVHDKhoa(Base):
    ...
    #thuoc tinh
    ma_hd = Column(Integer, ForeignKey("HoiDongKhoa.ma_hd", ondelete="CASCADE"), primary_key=True)
...
class HoiDongTruong(Base):
    ...
    tvhd_truong = relationship("TVHDTruong", back_populates="hd_truong", cascade="all, delete-orphan")

#model thành viên hội đồng trường
class TVHDTruong(Base):
    __tablename__ = "TVHDTruong"
    ...
    tu_cach = Column(Integer, nullable=False)
    ma_hd = Column(Integer, ForeignKey("HoiDongTruong.ma_hd", ondelete="CASCADE"), nullable=False)
    #relationship
...
class DangKyNCKH(Base):
    ...
    ma_sv = Column(String(20), ForeignKey("SinhVien.ma_sv", ondelete="CASCADE"), nullable=False)
    #relationship
...
class DeTaiNCKHSV(Base):
    ...
    diem_so = Column(Integer, nullable=True)
    #relationship
...
class TaiLieuNCKHSV(Base):
    ...
    ma_hd_khoa = Column(Integer, ForeignKey("HoiDongKhoa.ma_hd", ondelete="SET NULL"))
    ma_hd_truong = Column(Integer, ForeignKey("HoiDongTruong.ma_hd", ondelete="SET NULL"))
    ma_de_tai = Column(Integer, ForeignKey("DeTaiNCKHSV.ma_de_tai", ondelete="CASCADE"))
    #relationship
...
class NhomNCKH(Base):
    ...
    trang_thai = Column(Integer, nullable=False, default=1)
    ma_gv = Column(String(20), ForeignKey("GiangVien.ma_gv", ondelete="SET NULL"), nullable=True)
...
class DeTaiNCKHGV(Base):
    ...
    tv_de_tai_gv = relationship("ThanhVienNCKHGV", back_populates="de_tai_gv", cascade="all, delete-orphan")
...
class ThanhVienNCKHGV(Base):
    ...
    #thuoc phim
    ma_de_tai = Column(Integer, ForeignKey("DeTaiNCKHGV.ma_de_tai", ondelete="CASCADE"), primary_key=True)
    ma_gv = Column(String(20), ForeignKey("GiangVien.ma_gv", ondelete="CASCADE"), primary_key=True)
...
class TaiLieuNCKHGV(Base):
    ...
    trang_thai = Column(Integer, nullable=False, default=1)
    phan_hoi = Column(Text)
    ma_de_tai = Column(Integer, ForeignKey("DeTaiNCKHGV.ma_de_tai", ondelete="CASCADE"), nullable=False)
    ma_hd = Column(Integer, ForeignKey("HoiDongTruong.ma_hd", ondelete="SET NULL"), nullable=True)
    #relationship
...
class HocVi(Base):
    ...
    ma_gv = Column(String(20), ForeignKey("GiangVien.ma_gv", ondelete="CASCADE"), nullable=False)
    #relationship
...
class ChucDanhKhoaHoc(Base):
    ...
    ma_gv = Column(String(20), ForeignKey("GiangVien.ma_gv", ondelete="CASCADE"), nullable=False)
    #relationship
    giang_vien = relationship("GiangVien", back_populates="chuc_danh_khoa_hoc")
...
class TrinhDoHocVan(Base):
    ...
    ma_gv = Column(String(20), ForeignKey("GiangVien.ma_gv", ondelete="CASCADE"), nullable=False)
    #relationship
    giang_vien = relationship("GiangVien", back_populates="trinh_do_hoc_van")
...
class TrinhDoNgoaiNgu(Base):
    ...
    ma_gv = Column(String(20), ForeignKey("GiangVien.ma_gv", ondelete="CASCADE"), nullable=False)
    #relationship
    giang_vien = relationship("GiangVien", back_populates="trinh_do_ngoai_ngu")
...
class QuaTrinhCongTac(Base):
    ...
    ma_gv = Column(String(20), ForeignKey("GiangVien.ma_gv", ondelete="CASCADE"), nullable=False)
    #relationship
    giang_vien = relationship("GiangVien", back_populates="qua_trinh_cong_tac")
...
class SachBaoCongBo(Base):
    ...
    ma_gv = Column(String(20), ForeignKey("GiangVien.ma_gv", ondelete="CASCADE"), nullable=False)
    #relationship
    giang_vien = relationship("GiangVien", back_populates="sach_bao_cong_bo")
...
class PhatMinhSangChe(Base):
    ...
    ma_gv = Column(String(20), ForeignKey("GiangVien.ma_gv", ondelete="CASCADE"), nullable=False)
    #relationship
    giang_vien = relationship("GiangVien", back_populates="phat_minh_sang_che")
...
class HoatDongCaoHoc(Base):
    ...
    ma_gv = Column(String(20), ForeignKey("GiangVien.ma_gv", ondelete="CASCADE"), nullable=False)
    #relationship
    giang_vien = relationship("GiangVien", back_populates="hoat_dong_cao_hoc")
...
class HoatDongGiangDay(Base):
    ...
    ma_gv = Column(String(20), ForeignKey("GiangVien.ma_gv", ondelete="CASCADE"), nullable=False)
    #relationship
    giang_vien = relationship("GiangVien", back_populates="hoat_dong_giang_day")