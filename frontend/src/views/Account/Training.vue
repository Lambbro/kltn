      ...
      <div class="content">
        ...
        <div v-else class="column" v-show="canEdit">
          <div class="khoaDaoTao">
            <p class="section-title">Thêm thông tin khóa đào tạo/bồi dưỡng</p>
            <div class="info-row">
              <p class="sub-label">Tên khóa đào tạo</p>
              <input
                type="text"
                v-model="newEntry.ten_kdt"
                placeholder="Ví dụ: Khóa đào tạo CCNA"
                class="input-field"
              />
            </div>
            <div class="info-row">
              ...
              <input
                type="text"
                v-model="newEntry.noi_dao_tao"
                placeholder="Ví dụ: Cisco Networking Academy"
                class="input-field"
              />
            </div>
            <div class="info-row">
              ...
              <input
                ...
                placeholder="Ví dụ: 01/2023-03/2023"
                class="input-field"
              />
            </div>
            <div class="info-row">
              <p class="sub-label">Văn bằng/Chứng chỉ</p>
              <input
                type="text"
                v-model="newEntry.van_bang_chung_chi"
                placeholder="Ví dụ: CCNA v7"
                class="input-field"
              />
            </div>
        ...
        <div class="column">
          <div class="table-container">
            <p class="section-title">Các khóa đào tạo/bồi dưỡng khác</p>
            <table>
              <thead>
                <tr>
              ...
              <tbody>
                <tr v-for="(entry, index) in trainingEntries" :key="index">
                  <td>{{ entry.ten_kdt }}</td>
                  <td>{{ entry.noi_dao_tao }}</td>
                  <td>{{ entry.thoi_gian }}</td>
                  <td>{{ entry.van_bang_chung_chi }}</td>
                  <td v-if="canEdit">
...
// Lấy danh sách Khóa đào tạo từ API
async function fetchTrainingEntries() {
  try {
    ...
    const response = await api.get(`syll/khoadaotao/${ma_gv}`);
    trainingEntries.value = response.data || [];
  } catch (err) {
    handleError(err, 'Không thể tải thông tin khóa đào tạo');
  } finally {
    loading.value = false;
  }
}

// Hàm chỉnh sửa khóa đào tạo
...
async function addEntry() {
  if (
    !newEntry.value.ten_kdt ||
    !newEntry.value.noi_dao_tao ||
    !newEntry.value.thoi_gian ||
    !newEntry.value.van_bang_chung_chi
  ) {
  ...
  try {
    ...
    if (isEditing.value) {
      // Cập nhật thông tin
      await api.put(`syll/khoadaotao/${newEntry.value.ma_khoadt}`, payload);
      alert('Cập nhật khóa đào tạo thành công!');
    } else {
      // Thêm mới
      await api.post('syll/khoadaotao', payload);
    ...
    handleError(err, isEditing.value ? 'Không thể cập nhật khóa đào tạo' : 'Không thể thêm khóa đào tạo');
  } finally {
    saving.value = false;
  }
}
