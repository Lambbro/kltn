          ...
          <div v-if="!loading && !error" class="table-container">
            <table class="research-table">
              ...
              <tbody>
                ...
                <tr v-if="filteredResearchTopics.length === 0">
...
const deleteTopic = async (ma_hnc) => {
  if (confirm('Bạn có chắc chắn muốn xóa hướng nghiên cứu này?')) {
    try {
      await api.delete(`/delete/hnc/${ma_hnc}`);
...
const addTopic = async () => {
  try {
    ...
    // const response = await api.post('/add/hnc', newTopic.value);
    
    // Temporary mock response
    const mockResponse = {
      ma_hnc: researchTopics.value.length + 1,
      ten_hnc: newTopic.value.ten_hnc
    };
    
    researchTopics.value.push(mockResponse);
