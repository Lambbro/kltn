...
class NguyenVongDKBase(BaseModel):
    ...
    trang_thai: Optional[int] = Field(1, description="Trạng thái đăng ký (1: Đang xét duyệt, 2: Được duyệt, 3: Bị từ chối)")
...
class DeTaiNCKHSVBase(BaseModel):
    ...
    ma_khoa: str = Field(..., max_length=20, description="Mã khoa thực hiện")
...
class DeTaiNCKHSVUpdate(BaseModel):
    ...
    ma_khoa: Optional[str] = Field(None, max_length=20, description="Mã khoa thực hiện")
...
class TaiLieuNCKHSVBase(BaseModel):
    ...
    link_tep: str = Field(..., description="Đường dẫn file")
...
class NhomNCKHBase(BaseModel):
    ...
    ma_gv: str = Field(..., description="Mã giảng viên hướng dẫn")
