...
from models.base_models import DangKyNCKH, NguyenVongDK, SinhVien
from schemas.detai_sv_schemas import DangKyNguyenVongResponse

class KhoaDangKyService:
    def __init__(self, db: AsyncSession):
        self.db = db

    # LẤY TẤT CẢ ĐĂNG KÝ ĐANG THỰC HIỆN THEO MÃ KHOA (KÈM NGUYỆN VỌNG)
    async def khoa_get_all(self, ma_khoa) -> List[DangKyNguyenVongResponse]:
        try:
            result = await self.db.execute(
                select(DangKyNCKH)
                .join(SinhVien, DangKyNCKH.ma_sv == SinhVien.ma_sv)
                .filter(
            ...
            db_dks = result.scalars().unique().all()
    ...
    async def khoa_get_by_id(self, ma_dk: int, ma_khoa: str) -> DangKyNguyenVongResponse:
        try:
            # Lấy đăng ký theo ma_dk và trạng thái == 1 với nguyện vọng
            result = await self.db.execute(
                select(DangKyNCKH)
                .join(SinhVien, DangKyNCKH.ma_sv == SinhVien.ma_sv)
                .filter(DangKyNCKH.ma_dk == ma_dk and SinhVien.ma_khoa == ma_khoa)  # Lọc theo ma_dk và trang_thai = 1
                .options(joinedload(DangKyNCKH.nguyen_vong))  # Load danh sách nguyện vọng
            )
    ...
    async def khoa_get_by_ma_sv(self, ma_sv: str, ma_khoa: str) -> Optional[DangKyNguyenVongResponse]:
        db_dk = await self.db.execute(
            select(DangKyNCKH)
            .join(SinhVien, DangKyNCKH.ma_sv == SinhVien.ma_sv)
            .where(DangKyNCKH.ma_sv == ma_sv, DangKyNCKH.trang_thai == 1, SinhVien.ma_khoa == ma_khoa)  # Lọc theo ma_sv và trang_thai = 1
        )
